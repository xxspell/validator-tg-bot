...
import pymysql
from settings.reader import sql_host, sql_user, sql_database, sql_password

from tools.json_t import update_json_data, read_json_data
...
async def get_data_db():
    while True:
        try:
            ...
            cursor.execute("SELECT * FROM videos_untrusted WHERE checked = 0 ORDER BY time_listen LIMIT 1")
            row = cursor.fetchone()
            if row:
